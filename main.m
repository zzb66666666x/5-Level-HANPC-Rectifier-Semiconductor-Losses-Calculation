...
name='result.csv';
file=fopen(name,'w');
fprintf(file,'Switch,f_switch,I_amplitude,Alpha,N_num1,N_num2,P_S_switch_25,P_S_conduct,Eta_25,\r\n');
    ...
    for f_switch=900:50:3600%Hz
        for I_amplitude=1:-0.1:0.2%Percentage
            for Alpha=-10:1:10%Angle, but radian is required by calculation.m
                [N_num1,N_num2,P_S_switch_25,P_S_conduct,Eta_25]=calculation(f_switch,I_amplitude,Alpha/180*pi,switch_voltage(i),I_step);%Angle is converted to radian
                fprintf(file,"%s,%d,%f,%d,%d,%d,%f,%f,%f,\r\n",Switch,f_switch,I_amplitude,Alpha,N_num1,N_num2,P_S_switch_25,P_S_conduct,Eta_25);
            end
        end
    end
end